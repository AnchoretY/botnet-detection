...
in_channels = 1
enc_sizes = [32] * 12   # 各层编码向量维度
residual_hop = 1        # 每隔几层使用一个残差连接
edge_gate = 'none'      # 使用的数据聚合后的特殊处理方式
num_classes = 2         # 分类数
nodemodel = 'additive'  # 使用的基础图模型

nheads = [1]  # 多头attention数量，应该是一个长度为1或者等于层数的列表 should be a list of length 1 or equal to #layers
att_act = 'lrelu'  # 多头attention中使用的非线性激活函数
att_dropout = 0    # 多头attention中使用的dropout
att_dir = 'in'     # 多头注意力的方向，当我们的僵尸网络图特征少的时候使用out
att_combine = 'cat' # 多头注意力机制的向量合并方式

deg_norm = 'rw'    # 使用相邻节点的度更新更新节点信息时正则化方法，Symmetric和RandomWalk
aggr = 'add'       # 使用相邻节点的度更新更新节点信息时使用的整合方式
dropout = 0.0
bias = True
final = 'proj'     # 最后的输出层，'none', 'proj'
...
def parse_args():
    ...
    # ----------------------------- 整体设置 ------------------------------
    parser.add_argument('--devid', type=int, default=devid, help='device id; -1 for CPU') #驱动器编号
    parser.add_argument('--seed', type=int, default=seed, help='random seed')
    parser.add_argument('--logmode', type=str, default='w', help='logging file mode')   # 日志文件选择的模式
    parser.add_argument('--log_interval', type=int, default=96, help='logging interval during training') # 训练中每隔n轮输出一条日志
    # ---------------------------- 数据加载参数 ---------------------------
    ...
    parser.add_argument('--enc_sizes', type=int, nargs='*', default=enc_sizes, help='encoding node feature sizes') # 中间各层节点数量
    parser.add_argument('--act', type=str, default='relu', choices=['none', 'lrelu', 'relu', 'elu'],
                        help='non-linear activation function after adding residual')  # 残差层后面添加的非线性激活函数
    parser.add_argument('--layer_act', type=str, default='none', choices=['none', 'lrelu', 'relu', 'elu'],
                        help='non-linear activation function for each layer before residual')  # 残差层前面添加的非线性激活函数
    parser.add_argument('--residual_hop', type=int, default=residual_hop, help='residual per # layers')  # 每隔基层之间建立一个残差连接
    parser.add_argument('--edge_gate', type=str, choices=['none', 'proj', 'free'], default=edge_gate,
                        help='types of independent edge gate')   # 邻居节点信息汇总后的特别处理方式
    ...
    parser.add_argument('--nodemodel', type=str, default=nodemodel, choices=['additive', 'mlp', 'attention'],
                        help='name of node model class')    # 使用的基础模型类型（整体模型由参数很多基模型、残差等加和而成）
    parser.add_argument('--final', type=str, default=final, choices=['none', 'proj'], help='final output layer')  # 最后输出层
    # -------------------------- attention相关参数 ------------------------
    ...
    parser.add_argument('--att_act', type=str, default=att_act, choices=['none', 'lrelu', 'relu', 'elu'], 
                        help='attention activation function in multihead attention')  # 多头注意力机制中的非线性激活函数
    ...
    parser.add_argument('--att_dir', type=str, default=att_dir, choices=['in', 'out'],
                        help='attention direction in multihead attention') # 多头注意力的方向
    parser.add_argument('--att_combine', type=str, default=att_combine, choices=['cat', 'add', 'mean'],
                        help='multihead combination method in multihead attention')  # 多头注意力合并方式
    ...
    parser.add_argument('--deg_norm', type=str, choices=['none', 'sm', 'rw'], default=deg_norm,
                        help='degree normalization method')    # 使用相邻节点的度更新更新节点信息时正则化方法，Symmetric和RandomWalk
    parser.add_argument('--aggr', type=str, choices=['add', 'mean', 'max'], default=aggr,
                        help='feature aggregation method')     # 使用相邻节点的度更新更新节点信息时使用的整合方式
